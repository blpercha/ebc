    ...
    def run(self, assigned_clusters=None, verbose=True):
        ...
        for it in range(self.max_iters):
            ...
            if abs(objective - last_objective) < self.objective_tolerance:
                if verbose: print "EBC2D finished in %d iterations, with final objective value %.4f" % (it+1, objective)
                return [self.cX.tolist(), self.cY.tolist()], objective, it + 1
            last_objective = objective
        if verbose: print "EBC2D finished in %d iterations, with final objective value %.4f" % (self.max_iters, objective)
