...
for (file in list.files("/Users/Beth/Desktop/ddi-results", full.names=TRUE)) {
    ...
    size = as.numeric(as.character(strsplit(split.name[[1]][6], "\\.")[[1]][1]))
...
p2 <- ggplot(results.final, aes(x = S, y = fracg08, colour = method)) + geom_line(size = 0.75) + geom_point(aes(colour = method), size = 3) + xlab("Seed Set Size") + ylab("Fraction of Trials with AUC > 0.8") + xlim(0,100) + ylim(0,1) + labs(title = "(a)") + theme(plot.title = element_text(hjust = 0)) + theme(plot.title = element_text(lineheight=.8, face="bold")) + theme(panel.background = element_rect(fill = "gray95")) + scale_colour_manual(values = c("ebc3d" = colors[1], "bestcosine" = colors[2], "ranksum" = colors[3], "ebctotal" = colors[4], "ebcranksum" = colors[5]))

p3 <- ggplot(results.final, aes(x = S, y = fracg09, colour = method)) + geom_line(size = 0.75) + geom_point(aes(colour = method), size = 3) + xlab("Seed Set Size") + ylab("Fraction of Trials with AUC > 0.9") + xlim(0,100) + ylim(0,1) + labs(title = "(a)") + theme(plot.title = element_text(hjust = 0)) + theme(plot.title = element_text(lineheight=.8, face="bold")) + theme(panel.background = element_rect(fill = "gray95")) + scale_colour_manual(values = c("ebc3d" = colors[1], "bestcosine" = colors[2], "ranksum" = colors[3], "ebctotal" = colors[4], "ebcranksum" = colors[5]))
